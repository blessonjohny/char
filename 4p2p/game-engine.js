...
class GameEngine {
  ...
  _botAct(pos) {
    if (this.phase === 'bidding1' && this.currentPlayer === pos) {
      ...
      const strength = hand.reduce((s, c) => s + c.points, 0) + trumpCount * 2;
      const minBid = Math.max(20, this.highestBid + 1);
      // Bots are deliberately conservative about jumping into someone
      // else's bid/trump in phase 2 — only raise with a genuinely strong
      // hand relative to the trump suit already locked in.
      const willingTo = 18 + strength * 2;
