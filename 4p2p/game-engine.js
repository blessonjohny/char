...
class GameEngine {
  ...
  _botAct(pos) {
    if (this.phase === 'bidding1' && this.currentPlayer === pos) {
      ...
      const comfortThreshold = Math.max(0.35,
        0.75 - (b.level - 1) * 0.08 - (b.bidWeights.aggression - 1) * 0.1);
      ...
      // higher bar.
      const baseThreshold = Math.max(0.4, 0.8 - (b.level - 1) * 0.08 - (b.bidWeights.aggression - 1) * 0.1);
