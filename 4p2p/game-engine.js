...
class GameEngine {
  ...
  chooseTrump(pos, suit, hiddenCard) {
    ...
    this.dealCards(4);
    this.phase = 'play';
    this.currentPlayer = this.bidder; // bidder leads the first trick
  ...
  canPlayCard(pos, card) {
    ...
    if (this.trickSuit === '') {
      ...
      if (pos === this.bidder && !this.trumpExposed && card.suit === this.trumpSuit) {
  ...
  // The bidder may deliberately play their hidden card directly (exposing
  // trump in the process) instead of a card from their visible hand.
  playHiddenTrump(pos) {
    if (this.phase !== 'play') return { ok: false, reason: 'not_playing' };
    if (pos !== this.currentPlayer || pos !== this.bidder) return { ok: false, reason: 'not_your_turn' };
  ...
  exposeTrump() {
    ...
    if (this.hiddenTrump && this.seats[this.bidder]) {
      this.seats[this.bidder].hand.push(this.hiddenTrump);
      this.hiddenTrump = null;
    }
  }
  ...
  _resolveTrick() {
    ...
    if (cardsLeft === 0 && this.hiddenTrump) {
      // Everyone else is out of cards but the bidder's hidden card was
      // never exposed — it becomes their forced final play.
      this.currentPlayer = this.bidder;
      this._notify();
      this.maybeAutoAct();
    } else if (cardsLeft === 0) {
  ...
  _botAct(pos) {
    if (this.phase === 'bidding1' && this.currentPlayer === pos) {
      const hand = this.seats[pos].hand;
      const strength = hand.reduce((s, c) => s + c.points, 0) + hand.filter(c => RANK_ORDER[c.rank] >= 6).length;
      const first = this.isFirstBidder(pos);
      const minBid = this.highestBid > 0 ? this.highestBid + 1 : 14;
      const willingTo = 14 + strength * 2;
      ...
      if (legal.length === 0) {
        if (pos === this.bidder && this.hiddenTrump) { this.playHiddenTrump(pos); return; }
  ...
  stateFor(viewerPos) {
    return {
      ...
      trumpSuit: this.trumpExposed ? this.trumpSuit : (viewerPos === this.bidder ? this.trumpSuit : ''),
