...
function renderTrick(state) {
  ...
  zone.innerHTML = (state.trickCards || []).map(tc => cardHTML(tc.card, false, false, 'tiny')).join('');
}
...
function renderLastTrick(state) {
  ...
  if (!state.lastTrick || !state.lastTrick.cards || !state.lastTrick.cards.length) {
    el.innerHTML = '<span style="font-style:italic">None yet</span>';
    return;
  }
  const lt = state.lastTrick;
  const cardsHtml = lt.cards.map(tc => {
    const color = cardColor(tc.card.suit);
    return `<span style="color:${color};font-weight:800;margin-right:3px">${tc.card.rank}${tc.card.suit}</span>`;
  }).join('');
  const winnerSeat = state.seats[lt.winner];
  const winnerName = lt.winner === MY_POS ? 'You' : (winnerSeat ? winnerSeat.name : ('Seat ' + lt.winner));
  el.innerHTML = `<div style="line-height:1.6;word-break:break-all">${cardsHtml}</div><div style="color:var(--success);font-weight:700;margin-top:2px">${winnerName} +${lt.points}pt</div>`;
}
