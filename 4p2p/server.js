...
io.on('connection', (socket) => {
  ...
  socket.on('joinTable', ({ tableId: reqTableId, name, playerId: existingPlayerId }) => {
    ...
    if (!isPlaying) {
      ...
      socket.emit('chooseSeat', { tableId: reqTableId, openSeats, botSeats, disconnectedSeats, canWatch: false, needsApproval: false });
      return;
    }
  ...
  socket.on('respondJoinRequest', ({ reqId, approved }) => {
    ...
    reqSocket.emit('chooseSeat', { tableId, openSeats, botSeats, disconnectedSeats, canWatch: true, needsApproval: false });
  ...
  function rejectClaim(t, pending, reason) {
    ...
    if (openSeats.length > 0 || botSeats.length > 0 || disconnectedSeats.length > 0) {
      socket.emit('chooseSeat', { tableId: pending.tableId, openSeats, botSeats, disconnectedSeats, canWatch: true, needsApproval: false });
    }
  }
