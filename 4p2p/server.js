...
function broadcastTable(t) {
  ...
  if (t.spectators) {
    const spectatorState = t.engine.stateFor(-1); // never matches a real seat — naturally produces a hand-free view
    for (const [socketId] of t.spectators) {
      const sock = io.sockets.sockets.get(socketId);
      if (sock) sock.emit('state', spectatorState);
    }
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('joinTable', ({ tableId: reqTableId, name, playerId: existingPlayerId }) => {
    ...
    if (!isPlaying) {
      ...
      socket.emit('chooseSeat', { tableId: reqTableId, openSeats, botSeats: [], canWatch: false, needsApproval: false });
      return;
    }

    // Table is already playing — the host must approve this join before
    // any seat/watch choice is even offered.
    if (openSeats.length === 0 && botSeats.length === 0 && !canWatch) {
    ...
    for (const [hostSockId, info] of t.sockets) {
      ...
      if (hostSocket) {
        hostSocket.emit('joinRequest', {
          reqId, name: name || 'Player', openSeats, botSeats,
          tableFull: openSeats.length === 0 && botSeats.length === 0
        });
      }
    }
  ...
  socket.on('respondJoinRequest', ({ reqId, approved }) => {
    ...
    reqSocket.emit('chooseSeat', { tableId, openSeats, botSeats, canWatch: true, needsApproval: false });
  ...
  socket.on('claimSeat', ({ choice }) => {
    ...
    if (choice.type === 'openSeat') {
      if (!t.engine.emptySeats().includes(pos)) { socket.emit('joinError', { reason: 'seat_taken' }); return; }
      playerId = newId();
      t.engine.seatHuman(pos, pending.name, playerId);
    } else if (choice.type === 'replaceBot') {
      if (!t.engine.seats[pos] || !t.engine.seats[pos].isBot) { socket.emit('joinError', { reason: 'not_a_bot_seat' }); return; }
      playerId = newId();
      if (!t.engine.replaceBot(pos, playerId, pending.name)) { socket.emit('joinError', { reason: 'replace_failed' }); return; }
    } else {
      return;
    }
    tableId = pending.tableId;
